#!/usr/bin/env python3
"""Fit CMO telecentric model with SE(3) arm alignment.

Jointly optimizes telecentric parameters + per-channel SE(3) transforms
against the stable Zernike rayfield. Evaluates pixel reprojection.
"""
...
# ═══════════════════════════════════════════════════════════
# Pipeline (detection + Hessian + double TPS + Zernike)
# ═══════════════════════════════════════════════════════════
...
print("=" * 60)
print("1 — Detection + Hessian + double TPS")
print("=" * 60)
...
# ═══════════════════════════════════════════════════════════
# Joint fit: telecentric params + SE(3) arm alignment
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("2 — Joint fit: telecentric + SE(3) arm alignment")
print("=" * 60)
...
# Parameter layout:
# [0:14] telecentric base (f_obj, WD, b, cx, cy, f_ang, theta, dy, sxL, syL, sxR, syR, rho_x, rho_y)
# [14:20] SE(3) left  (rv_L 3, t_L 3)
# [20:26] SE(3) right (rv_R 3, t_R 3)

def apply_se3(O, d, rv, t):
    """Apply SE(3) to rays: O' = R@O + t, d' = normalize(R@d)."""
    R = Rotation.from_rotvec(rv).as_matrix()
    O_new = (R @ O.T).T + t[None, :]
    d_new = _normalize((R @ d.T).T)
    return O_new, d_new

def model_rays(x, uf, vf):
    """Evaluate telecentric + SE(3) aligned rays."""
    tel_x = x[:14]
    m_tel = CMOTelecentricStereoModel.from_parameter_vector(tel_x, pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    OL, dL = m_tel.ray(uf, vf, "left")
    OR, dR = m_tel.ray(uf, vf, "right")
    rv_L, t_L = x[14:17], x[17:20]
    rv_R, t_R = x[20:23], x[23:26]
    OL_a, dL_a = apply_se3(OL, dL, rv_L, t_L)
    OR_a, dR_a = apply_se3(OR, dR, rv_R, t_R)
    return OL_a, dL_a, OR_a, dR_a

# Grid for rayfield evaluation
...
def residuals(x):
    OL_a, dL_a, OR_a, dR_a = model_rays(x, uf, vf)
    # Two-plane: compare aligned rays against Zernike at z=50,80 mm
    blocks = []
    for z in [50., 80.]:
        tz_refL = (z - OzL[:, 2]) / dzL[:, 2]
        P_refL = OzL + tz_refL[:, None] * dzL
        tz_modL = (z - OL_a[:, 2]) / dL_a[:, 2]
        P_modL = OL_a + tz_modL[:, None] * dL_a
        blocks.append((P_refL - P_modL).reshape(-1))

        tz_refR = (z - OzR[:, 2]) / dzR[:, 2]
        P_refR = OzR + tz_refR[:, None] * dzR
        tz_modR = (z - OR_a[:, 2]) / dR_a[:, 2]
        P_modR = OR_a + tz_modR[:, None] * dR_a
        blocks.append((P_refR - P_modR).reshape(-1))
    return np.concatenate(blocks)

# Initial: telecentric fit first, then add zero SE(3)
# Fit telecentric only
def res_tel_only(x_tel):
    m = CMOTelecentricStereoModel.from_parameter_vector(x_tel, pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    l = m.channel("left"); r = m.channel("right")
...
sol_tel = least_squares(res_tel_only, x0=x0_tel, bounds=(lo_tel, hi_tel), loss="linear", max_nfev=500,
                         xtol=1e-10, ftol=1e-10, gtol=1e-10)
tel_x_opt = sol_tel.x
print(f"Telecentric only: {tel_x_opt.size} params, RMS={float(np.sqrt(np.mean(res_tel_only(tel_x_opt)**2))):.4f} mm")

# Now joint fit
x0 = np.concatenate([tel_x_opt, np.zeros(12)])  # 14 tele + 12 SE(3) = 26
lo = np.concatenate([lo_tel, np.full(6, -0.05), np.full(6, -2.0)])   # ±3° rot, ±2mm trans
hi = np.concatenate([hi_tel, np.full(6, 0.05), np.full(6, 2.0)])

print(f"\nJoint fit: {x0.size} params (14 tele + 12 SE(3))")
t0 = time.time()
sol = least_squares(residuals, x0=x0, bounds=(lo, hi), loss="huber", f_scale=1.0, max_nfev=1000,
                     xtol=1e-10, ftol=1e-10, gtol=1e-10)
elapsed = time.time() - t0
rms_joint = float(np.sqrt(np.mean(residuals(sol.x)**2)))
print(f"  Converged: {sol.success}, nfev={sol.nfev}, time={elapsed:.0f}s, RMS={rms_joint:.4f} mm")

# Extract results
x_opt = sol.x
rv_L = x_opt[14:17]; t_L = x_opt[17:20]
rv_R = x_opt[20:23]; t_R = x_opt[23:26]
ang_L = float(np.linalg.norm(rv_L)); ang_R = float(np.linalg.norm(rv_R))
print(f"  SE(3) L: rot={np.degrees(ang_L):.3f}°  t=[{t_L[0]:.3f}, {t_L[1]:.3f}, {t_L[2]:.3f}] mm")
print(f"  SE(3) R: rot={np.degrees(ang_R):.3f}°  t=[{t_R[0]:.3f}, {t_R[1]:.3f}, {t_R[2]:.3f}] mm")

# Direction and moment residuals after alignment
OL_a, dL_a, OR_a, dR_a = model_rays(x_opt, uf, vf)
dot_L = np.clip(np.sum(dzL*dL_a, axis=1), -1, 1)
dot_R = np.clip(np.sum(dzR*dR_a, axis=1), -1, 1)
...
mom_rms_L = float(np.sqrt(np.mean(np.linalg.norm(mzL-maL, axis=1)**2)))
mom_rms_R = float(np.sqrt(np.mean(np.linalg.norm(mzR-maR, axis=1)**2)))
print(f"  Dir RMS: L={dir_rms_L:.4f}°  R={dir_rms_R:.4f}°")
print(f"  Mom RMS: L={mom_rms_L:.4f} mm  R={mom_rms_R:.4f} mm")

# ═══════════════════════════════════════════════════════════
# 3 — Pixel reprojection
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("3 — Pixel reprojection")
print("=" * 60)

m_tel_opt = CMOTelecentricStereoModel.from_parameter_vector(x_opt[:14], pixel_pitch_mm=0.0055, image_size=IMG_SIZE)

def reproj_px(use_alignment=True):
    """Compute pixel-equivalent reprojection error."""
    epx = []
    for pi in range(n_frames):
        ...
        for k in range(obj_pts.shape[0]):
            for uv, ch in [(left_pixels[pi][k], "left"), (right_pixels[pi][k], "right")]:
                O_tel, d_tel = m_tel_opt.ray(np.array([uv[0]]), np.array([uv[1]]), ch)
                if use_alignment:
                    rv = rv_L if ch == "left" else rv_R
                    t_arm = t_L if ch == "left" else t_R
                    O_use, d_use = apply_se3(O_tel.reshape(1, 3), d_tel.reshape(1, 3), rv, t_arm)
                    O_use, d_use = O_use[0], d_use[0]
                else:
...
px_rms_tel, px_p50_tel, px_p95_tel = reproj_px(use_alignment=False)
px_rms_alg, px_p50_alg, px_p95_alg = reproj_px(use_alignment=True)

print(f"  Telecentric only (14p):  RMS={px_rms_tel:.2f} px  P50={px_p50_tel:.2f} px  P95={px_p95_tel:.2f} px")
print(f"  + SE(3) aligned (26p):   RMS={px_rms_alg:.2f} px  P50={px_p50_alg:.2f} px  P95={px_p95_alg:.2f} px")

# Summary table
print(f"\n{'─'*70}")
print(f"{'Model':>30s}  {'Params':>6s}  {'Ray RMS':>9s}  {'Px RMS':>7s}  {'Px P50':>7s}  {'Px P95':>7s}")
print(f"  {'─'*30}  {'─'*6}  {'─'*9}  {'─'*7}  {'─'*7}  {'─'*7}")
print(f"  {'telecentric (L0)':>30s}  {14:>6d}  {float(np.sqrt(np.mean(res_tel_only(tel_x_opt)**2))):9.4f}  {px_rms_tel:7.2f}  {px_p50_tel:7.2f}  {px_p95_tel:7.2f}")
print(f"  {'telecentric + SE(3) arm':>30s}  {26:>6d}  {rms_joint:9.4f}  {px_rms_alg:7.2f}  {px_p50_alg:7.2f}  {px_p95_alg:7.2f}")
print(f"  {'Zernike O(0)+d(2)':>30s}  {57:>6d}  {0.0007:9.4f}  {0.47:7.2f}  {'─':>7s}  {'─':>7s}")

# Save
artifact = {
    "description": "Joint telecentric + SE(3) arm alignment fit vs Zernike rayfield",
    "telecentric_14p": {"ray_rms_mm": float(np.sqrt(np.mean(res_tel_only(tel_x_opt)**2))),
                        "px_rms": px_rms_tel, "px_p50": px_p50_tel, "px_p95": px_p95_tel},
    "aligned_26p": {"ray_rms_mm": rms_joint, "px_rms": px_rms_alg, "px_p50": px_p50_alg, "px_p95": px_p95_alg,
                    "arm_L": {"rot_deg": float(np.degrees(ang_L)), "t_mm": [float(v) for v in t_L]},
                    "arm_R": {"rot_deg": float(np.degrees(ang_R)), "t_mm": [float(v) for v in t_R]},
                    "dir_rms_deg": {"L": dir_rms_L, "R": dir_rms_R},
                    "mom_rms_mm": {"L": mom_rms_L, "R": mom_rms_R}},
}
with open(OUT / "aligned_cmo_fit.json", "w") as f:
    json.dump(artifact, f, indent=2)
print(f"\nSaved: {OUT / 'aligned_cmo_fit.json'}")
print("Done!")
