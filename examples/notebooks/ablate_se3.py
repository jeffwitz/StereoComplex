#!/usr/bin/env python3
"""SE(3) arm alignment ablation: find minimal parameter set.

Tests: rotations only, translations only, differential SE(3),
symmetrized SE(3), vs full SE(3) L/R reference.
"""
...
# ═══════════════════════════════════════════════════════════
# Pipeline (same as before, abbreviated)
# ═══════════════════════════════════════════════════════════
...
def build_eval(x_tel, arm_params, variant):
    """Build model rays given telecentric params and arm params."""
    ...
    if variant == "rot_only":
        rv_L, rv_R = arm_params[:3], arm_params[3:6]
        OL_a, dL_a = apply_se3(OL, dL, rv_L, np.zeros(3))
        OR_a, dR_a = apply_se3(OR, dR, rv_R, np.zeros(3))
    elif variant == "trans_only":
        t_L, t_R = arm_params[:3], arm_params[3:6]
        OL_a, dL_a = apply_se3(OL, dL, np.zeros(3), t_L)
        OR_a, dR_a = apply_se3(OR, dR, np.zeros(3), t_R)
    elif variant == "diff_se3":
        # Differential SE(3): common SE(3) + relative
        rv_common, t_common = arm_params[:3], arm_params[3:6]
        rv_rel, t_rel = arm_params[6:9], arm_params[9:12]
        R_common = Rotation.from_rotvec(rv_common).as_matrix()
        R_rel = Rotation.from_rotvec(rv_rel).as_matrix()
        # L = common, R = common ∘ rel
        R_L = R_common; t_L = t_common
        R_R = R_common @ R_rel; t_R = t_common + t_rel
        OL_a = (R_L @ OL.T).T + t_L[None, :]; dL_a = _normalize((R_L @ dL.T).T)
        OR_a = (R_R @ OR.T).T + t_R[None, :]; dR_a = _normalize((R_R @ dR.T).T)
    elif variant == "sym_se3":
        # CMO-symmetric: rotation = mean ± stereo, translation = ±antisym
        rv_mean, rv_stereo = arm_params[:3], arm_params[3:6]
        t_mean, t_antisym = arm_params[6:9], arm_params[9:12]
        ...
        R_L = R_mean @ R_stereo; t_L = t_mean + t_antisym
        R_R = R_mean @ R_stereo_inv; t_R = t_mean - t_antisym
        OL_a = (R_L @ OL.T).T + t_L[None, :]; dL_a = _normalize((R_L @ dL.T).T)
        OR_a = (R_R @ OR.T).T + t_R[None, :]; dR_a = _normalize((R_R @ dR.T).T)
    elif variant == "full_se3":
...
def make_residuals(variant, n_arm):
    def residuals(x):
        x_tel, arm_params = x[:14], x[14:14+n_arm]
        OL_a, dL_a, OR_a, dR_a = build_eval(x_tel, arm_params, variant)
        blocks = []
        for z in [50., 80.]:
...
def fit_variant(name, variant, n_arm, arm_lo, arm_hi):
    """Fit one ablation variant."""
    x0 = np.concatenate([tel_x_opt, np.zeros(n_arm)])
    lo = np.concatenate([lo_tel, arm_lo])
    hi = np.concatenate([hi_tel, arm_hi])
    res = make_residuals(variant, n_arm)
    t0 = time.time()
    sol = least_squares(res, x0=x0, bounds=(lo, hi), loss="huber", f_scale=1.0, max_nfev=500,
                         xtol=1e-10, ftol=1e-10, gtol=1e-10)
    elapsed = time.time() - t0
    rms = float(np.sqrt(np.mean(res(sol.x)**2)))

    # Pixel reprojection
    m_tel = CMOTelecentricStereoModel.from_parameter_vector(sol.x[:14], pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    arm = sol.x[14:]
    epx = []
    for pi in range(n_frames):
        ...
        for k in range(obj_pts.shape[0]):
            for uv, ch in [(left_pixels[pi][k], "left"), (right_pixels[pi][k], "right")]:
                ...
                # Apply arm transform
                if variant == "rot_only":
                    rv = arm[:3] if ch == "left" else arm[3:6]
                    t_arm = np.zeros(3)
                elif variant == "trans_only":
                    rv = np.zeros(3)
                    t_arm = arm[:3] if ch == "left" else arm[3:6]
                elif variant == "diff_se3":
                    rv_c, t_c = arm[:3], arm[3:6]; rv_rel, t_rel = arm[6:9], arm[9:12]
                    if ch == "left": rv, t_arm = rv_c, t_c
                    else:
                        R_c = Rotation.from_rotvec(rv_c).as_matrix()
                        R_rel = Rotation.from_rotvec(rv_rel).as_matrix()
                        rv = Rotation.from_matrix(R_c @ R_rel).as_rotvec()
                        t_arm = t_c + t_rel
                elif variant == "sym_se3":
                    rv_m, rv_s = arm[:3], arm[3:6]; t_m, t_a = arm[6:9], arm[9:12]
                    R_m = Rotation.from_rotvec(rv_m).as_matrix()
                    R_s = Rotation.from_rotvec(rv_s).as_matrix()
                    if ch == "left":
                        rv = Rotation.from_matrix(R_m @ R_s).as_rotvec(); t_arm = t_m + t_a
                    else:
                        rv = Rotation.from_matrix(R_m @ Rotation.from_matrix(R_s).inv().as_matrix()).as_rotvec(); t_arm = t_m - t_a
                elif variant == "full_se3":
                    rv = arm[:3] if ch == "left" else arm[6:9]
                    t_arm = arm[3:6] if ch == "left" else arm[9:12]
                else:
                    rv, t_arm = np.zeros(3), np.zeros(3)
    ...
    return {
        "name": name, "variant": variant, "n_params": 14 + n_arm,
        "ray_rms": rms, "px_rms": float(np.sqrt(np.mean(epx_arr**2))),
        "px_p50": float(np.percentile(epx_arr, 50)), "px_p95": float(np.percentile(epx_arr, 95)),
        "converged": bool(sol.success), "nfev": int(sol.nfev), "time_s": elapsed,
        "arm_params": [float(v) for v in arm],
    }

# First, get telecentric-only baseline
...
def res_tel_only(x):
    ...
    return np.concatenate([rayfield_two_plane_residuals(lf, l, _grid_pixels(IMG_SIZE,(12,9)), z_planes=(50.,80.)),
                           rayfield_two_plane_residuals(rf, r, _grid_pixels(IMG_SIZE,(12,9)), z_planes=(50.,80.))])

sol_tel = least_squares(res_tel_only, x0=x0_tel, bounds=(lo_tel, hi_tel), loss="linear", max_nfev=300, xtol=1e-10)
tel_x_opt = sol_tel.x

# Ablation variants
rot_lo = np.full(3, -0.08); rot_hi = np.full(3, 0.08)    # ±4.6°
trans_lo = np.full(3, -3.0); trans_hi = np.full(3, 3.0)   # ±3 mm

variants = [
    ("rot_only",     "rot_only",     6,  np.concatenate([rot_lo, rot_lo]),
                                         np.concatenate([rot_hi, rot_hi])),
    ("trans_only",   "trans_only",   6,  np.concatenate([trans_lo, trans_lo]),
                                         np.concatenate([trans_hi, trans_hi])),
    ("full_se3",     "full_se3",    12,  np.concatenate([rot_lo, trans_lo, rot_lo, trans_lo]),
                                         np.concatenate([rot_hi, trans_hi, rot_hi, trans_hi])),
    ("diff_se3",     "diff_se3",    12,  np.concatenate([rot_lo, trans_lo, rot_lo, trans_lo]),
                                         np.concatenate([rot_hi, trans_hi, rot_hi, trans_hi])),
    ("sym_se3",      "sym_se3",     12,  np.concatenate([rot_lo, rot_lo, trans_lo, trans_lo]),
                                         np.concatenate([rot_hi, rot_hi, trans_hi, trans_hi])),
]

print(f"\n{'Model':>20s}  {'P':>4s}  {'RayRMS':>8s}  {'PxRMS':>7s}  {'P50':>7s}  {'P95':>7s}  {'NFEV':>5s}  {'s':>4s}")
print("-" * 80)
results = []
for name, variant, n_arm, arm_lo, arm_hi in variants:
    r = fit_variant(name, variant, n_arm, arm_lo, arm_hi)
    results.append(r)
    print(f"  {name:>20s}  {r['n_params']:>4d}  {r['ray_rms']:8.4f}  {r['px_rms']:7.2f}  {r['px_p50']:7.2f}  {r['px_p95']:7.2f}  {r['nfev']:>5d}  {r['time_s']:4.0f}")

# Telecentric baseline pix reproj
tel_rms = float(np.sqrt(np.mean(res_tel_only(tel_x_opt)**2)))
print(f"  {'telecentric':>20s}  {14:>4d}  {tel_rms:8.4f}  {'~14.6':>7s}  {'─':>7s}  {'─':>7s}  {'─':>5s}  {'─':>4s}")

# Best candidate
best = min(results, key=lambda r: r["px_rms"])
print(f"\nBest: {best['name']} — {best['px_rms']:.2f} px ({best['n_params']} params)")

# Save
with open(OUT / "se3_ablation.json", "w") as f:
    json.dump({"description": "SE(3) arm alignment ablation study", "results": results}, f, indent=2)
print(f"Saved: {OUT / 'se3_ablation.json'}")
print("Done!")
