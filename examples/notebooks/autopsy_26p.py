#!/usr/bin/env python3
"""Autopsy of the 26p aligned CMO: residual analysis + minimal extensions."""

from __future__ import annotations
import json, sys, time, math
from pathlib import Path
import numpy as np
ROOT = Path(__file__).resolve().parent.parent.parent
SRC = ROOT / "src"
sys.path.insert(0, str(SRC))
OUT = ROOT / "docs" / "assets" / "pycaso_real_data"
OUT.mkdir(parents=True, exist_ok=True)

import cv2
from cv2 import aruco
from scipy.optimize import least_squares
from scipy.spatial.transform import Rotation

from stereocomplex.core.rayfield2d import predict_points_rayfield_tps_robust
from stereocomplex.benchmarks.charuco_observation_simulator import CharucoObservationSet
from stereocomplex.benchmarks.rayfield_from_observations import fit_constrained_zernike_rayfield
from stereocomplex.physics.cmo_physical import CMOTelecentricStereoModel, _ray_rms, _normalize
from stereocomplex.physics.model_selection import rayfield_two_plane_residuals, _grid_pixels
from stereocomplex.core.model_compact.zernike import zernike_modes, eval_real_zernike

# ═══════════════════════════════════════════════════════════
# Pipeline
# ═══════════════════════════════════════════════════════════
PYCASO = ROOT / "examples" / "pycaso_data"
LEFT_DIR = PYCASO / "Exemple" / "Images_example" / "left_calibration11"
RIGHT_DIR = PYCASO / "Exemple" / "Images_example" / "right_calibration11"
NCX, NCY, SQR = 16, 12, 0.3
IMG_SIZE = (2048, 2048); W, H = IMG_SIZE
FX = 25600.0
K = np.array([[FX, 0, 1024], [0, FX, 1024], [0, 0, 1]], dtype=np.float64)

dictionary = aruco.getPredefinedDictionary(getattr(aruco, "DICT_6X6_250"))
ocv_board = aruco.CharucoBoard((NCX, NCY), SQR, SQR / 2, dictionary)
ocv_board.setLegacyPattern(True)
chess3 = ocv_board.getChessboardCorners()
board_ids = ocv_board.getIds().ravel()
board_obj = ocv_board.getObjPoints()
id_to_obj = {int(board_ids[i]): np.asarray(board_obj[i], dtype=np.float64)[:, :2] for i in range(len(board_ids))}

params = aruco.DetectorParameters()
params.cornerRefinementMethod = aruco.CORNER_REFINE_SUBPIX
params.adaptiveThreshWinSizeMin, params.adaptiveThreshWinSizeMax = 3, 75
params.adaptiveThreshWinSizeStep = 4
params.minMarkerPerimeterRate, params.maxMarkerPerimeterRate = 0.005, 0.20
params.polygonalApproxAccuracyRate, params.minCornerDistanceRate = 0.08, 0.02
params.minDistanceToBorder = 1
aruco_det = aruco.ArucoDetector(dictionary, params)
charuco_det = aruco.CharucoDetector(ocv_board)

def abs_det_hessian(gray, sigma=9.0):
    f = gray.astype(np.float32)
    if f.max() > 2: f /= 255.0
    f = cv2.GaussianBlur(f, (0, 0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REPLICATE)
    return np.abs(cv2.Sobel(f, cv2.CV_64F, 2, 0, ksize=3)*cv2.Sobel(f, cv2.CV_64F, 0, 2, ksize=3)
                  - cv2.Sobel(f, cv2.CV_64F, 1, 1, ksize=3)**2)

def otsu_mask(response):
    r8 = cv2.normalize(response, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
    _, mask = cv2.threshold(r8, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    return mask

def blob_barycentre(mask, xi, yi, d, prefer_largest=True):
    h, w = mask.shape
    x0, x1 = max(0, int(xi)-d), min(w, int(xi)+d)
    y0, y1 = max(0, int(yi)-d), min(h, int(yi)+d)
    if x1 <= x0+2 or y1 <= y0+2: return math.nan, math.nan, math.nan
    roi = (mask[y0:y1, x0:x1] > 0).astype(np.uint8)
    nl, labels, stats, _ = cv2.connectedComponentsWithStats(roi, connectivity=8)
    if nl <= 1: return math.nan, math.nan, math.nan
    if prefer_largest: k = 1 + int(np.argmax(stats[1:, cv2.CC_STAT_AREA]))
    else:
        cx_c, cy_c = xi-x0, yi-y0; best_k, best_d2 = 1, float("inf")
        for k in range(1, nl):
            sx, sy = stats[k, cv2.CC_STAT_LEFT], stats[k, cv2.CC_STAT_TOP]
            sw, sh = stats[k, cv2.CC_STAT_WIDTH], stats[k, cv2.CC_STAT_HEIGHT]
            d2 = (sx+sw/2-cx_c)**2 + (sy+sh/2-cy_c)**2
            if d2 < best_d2: best_d2, best_k = d2, k
        k = best_k
    m = cv2.moments((labels == k).astype(np.uint8), binaryImage=True)
    if m["m00"] < 1e-10: return math.nan, math.nan, math.nan
    return (m["m10"]/m["m00"]+x0, m["m01"]/m["m00"]+y0, float(m["m00"]))

def win_spot_2pass(mask, l_step, d_init, xi, yi, prefer_largest):
    for d_search in [d_init, max(d_init, int(l_step*0.5))]:
        xd, yd, area = blob_barycentre(mask, xi, yi, d_search, prefer_largest)
        if not math.isnan(xd): return xd, yd, area
    return math.nan, math.nan, math.nan

def ids_to_grid(ids, ncx=16):
    ids_arr = np.asarray(ids, dtype=np.float32).reshape(-1); nx = ncx - 1
    return np.column_stack([ids_arr % nx, ids_arr // nx]).astype(np.float32)

def fit_affine(img_pts, ids_arr, ncx=16):
    img = np.asarray(img_pts, dtype=np.float32).reshape(-1, 2)
    grid = ids_to_grid(np.asarray(ids_arr, dtype=np.int32).reshape(-1), ncx)
    A, _ = cv2.estimateAffine2D(grid, img, method=cv2.LMEDS)
    if A is None:
        X = np.column_stack([grid, np.ones(len(grid), dtype=np.float32)])
        A = np.linalg.lstsq(X, img, rcond=None)[0].T.astype(np.float32)
    return A

def project_affine(A, ids, ncx=16):
    grid = ids_to_grid(np.asarray(ids, dtype=np.int32).reshape(-1), ncx)
    return (np.column_stack([grid, np.ones(len(grid), dtype=np.float32)])) @ A.T

def complete_corners_hessian(gray, cc, ids, ncx, ncy, marker_corners=None, marker_ids=None):
    nx, ny = ncx-1, ncy-1; n_corners = nx * ny
    R = abs_det_hessian(gray); mask = otsu_mask(R)
    detected = {}
    if ids is not None and len(ids) > 0:
        for i in range(len(np.asarray(ids).ravel())):
            detected[int(np.asarray(ids).ravel()[i])] = np.asarray(cc).reshape(-1, 2)[i].astype(np.float64)
    cids = sorted(detected.keys())
    pred_xy = None
    if marker_corners is not None and marker_ids is not None:
        obj_xy_list, img_uv_list = [], []
        for i in range(len(marker_ids)):
            mid = int(marker_ids[i].ravel()[0]); o = id_to_obj.get(mid)
            if o is None: continue
            mc = np.asarray(marker_corners[i], dtype=np.float64).reshape(-1, 2)
            if mc.shape[0] == 4: obj_xy_list.append(o); img_uv_list.append(mc)
        if len(obj_xy_list) >= 4:
            try:
                pred_xy = predict_points_rayfield_tps_robust(
                    np.concatenate(obj_xy_list, axis=0), np.concatenate(img_uv_list, axis=0),
                    chess3[:, :2].astype(np.float64), lam=10.0, huber_c=3.0, iters=3, ransac_reproj_px=3.0)
            except Exception: pred_xy = None
    if pred_xy is None and len(cids) >= 3:
        A = fit_affine(np.array([detected[i] for i in cids]), np.array(cids), ncx)
        pred_xy = project_affine(A, np.arange(n_corners), ncx)
    l_step = 50.0
    if len(cids) >= 2:
        dp = float(np.linalg.norm(detected[cids[-1]] - detected[cids[0]]))
        g0, g1 = ids_to_grid(np.array([cids[0]]), ncx)[0], ids_to_grid(np.array([cids[-1]]), ncx)[0]
        dg = float(np.linalg.norm(g1 - g0))
        if dg > 1e-8: l_step = dp / dg
    d_init = max(3, int(l_step * 0.3))
    if len(cids) > 0:
        xA, yA = float(detected[cids[0]][0]), float(detected[cids[0]][1])
        _, _, a_test = win_spot_2pass(mask, l_step, int(l_step*2/3), xA, yA, True)
        if not math.isnan(a_test) and float(a_test) > 0: d_init = max(3, int(math.sqrt(float(a_test))))
    result = np.full((n_corners, 2), np.nan)
    for idx in range(n_corners):
        if idx in detected: result[idx] = detected[idx]
        else:
            xi, yi = float(pred_xy[idx, 0]), float(pred_xy[idx, 1])
            xd, yd, _ = win_spot_2pass(mask, l_step, d_init, xi, yi, False)
            if not math.isnan(xd): result[idx] = [float(xd), float(yd)]
    for idx in range(n_corners):
        if np.isnan(result[idx, 0]): result[idx] = [float(pred_xy[idx, 0]), float(pred_xy[idx, 1])]
    return result

print("=" * 60)
print("Pipeline: detection + Hessian + double TPS + Zernike")
print("=" * 60)
lz = sorted([f.stem for f in LEFT_DIR.iterdir() if f.suffix == ".png"], key=float)
rz = sorted([f.stem for f in RIGHT_DIR.iterdir() if f.suffix == ".png"], key=float)
paired_z = sorted(set(lz) & set(rz), key=float)
n_frames = len(paired_z)

denoised_L, denoised_R, detection_counts = [], [], []
for z_str in paired_z:
    lg = cv2.imread(str(LEFT_DIR / f"{z_str}.png"), 0)
    rg = cv2.imread(str(RIGHT_DIR / f"{z_str}.png"), 0)
    cc_L, ids_L, _, _ = charuco_det.detectBoard(lg)
    cc_R, ids_R, _, _ = charuco_det.detectBoard(rg)
    nL = 0 if ids_L is None else len(ids_L); nR = 0 if ids_R is None else len(ids_R)
    mk_c_L, mk_ids_L = aruco_det.detectMarkers(lg)[:2]
    mk_c_R, mk_ids_R = aruco_det.detectMarkers(rg)[:2]
    comp_L = complete_corners_hessian(lg, cc_L, ids_L, NCX, NCY, mk_c_L, mk_ids_L)
    comp_R = complete_corners_hessian(rg, cc_R, ids_R, NCX, NCY, mk_c_R, mk_ids_R)
    dmask_L = np.zeros(165, dtype=bool)
    dmask_R = np.zeros(165, dtype=bool)
    if ids_L is not None: dmask_L[np.asarray(ids_L).ravel()] = True
    if ids_R is not None: dmask_R[np.asarray(ids_R).ravel()] = True
    detection_counts.append((dmask_L, dmask_R))
    for mk_c, mk_ids, comp, out in [(mk_c_L, mk_ids_L, comp_L, denoised_L), (mk_c_R, mk_ids_R, comp_R, denoised_R)]:
        obj_xy_list, img_uv_list = [], []
        if mk_ids is not None:
            for i in range(len(mk_ids)):
                mid = int(mk_ids[i].ravel()[0]); o = id_to_obj.get(mid)
                if o is None: continue
                mc = np.asarray(mk_c[i], dtype=np.float64).reshape(-1, 2)
                if mc.shape[0] == 4: obj_xy_list.append(o); img_uv_list.append(mc)
        if len(obj_xy_list) >= 4:
            pred = predict_points_rayfield_tps_robust(
                np.concatenate(obj_xy_list, axis=0), np.concatenate(img_uv_list, axis=0),
                chess3[:, :2].astype(np.float64), lam=10.0, huber_c=3.0, iters=3, ransac_reproj_px=3.0)
        else: pred = comp
        re_denoised = predict_points_rayfield_tps_robust(
            chess3[:, :2].astype(np.float64), pred.astype(np.float64),
            chess3[:, :2].astype(np.float64), lam=3.0, huber_c=1.5, iters=2, ransac_reproj_px=2.0)
        out.append(re_denoised)
    print(f"  {z_str}: L {nL}→165  R {nR}→165")

obj_pts = chess3.astype(np.float64)
left_pixels = [dn.astype(np.float64) for dn in denoised_L]
right_pixels = [dn.astype(np.float64) for dn in denoised_R]
point_indices = [np.arange(165, dtype=int) for _ in range(n_frames)]
rvecs, tvecs = [], []
for lp in left_pixels:
    s, rv, tv = cv2.solvePnP(obj_pts.astype(np.float32), lp.astype(np.float32), K.astype(np.float32), np.zeros(5, dtype=np.float32))
    rvecs.append(rv.ravel().astype(np.float64) if s else np.zeros(3))
    tvecs.append(tv.ravel().astype(np.float64) if s else np.array([0., 0., 65.]))
obs = CharucoObservationSet(object_points_mm=obj_pts, pose_rvecs=np.array(rvecs), pose_tvecs=np.array(tvecs),
                             left_pixels=left_pixels, right_pixels=right_pixels, point_indices=point_indices,
                             noise_std_px=0.0, image_size=IMG_SIZE)
t0 = time.time()
lf, rf, zd, opt_R, opt_t = fit_constrained_zernike_rayfield(obs, image_size=IMG_SIZE, K_left=K, K_right=K.copy(),
                                                              max_order_d=2, max_nfev=500, origin_reg_weight=0.0)
print(f"Zernike: {zd.ray_rms_mm:.6f} mm  {time.time()-t0:.0f}s")

# Common setup
u_grid, v_grid = np.meshgrid(np.linspace(0, W-1, 41), np.linspace(0, H-1, 41))
uf, vf = u_grid.ravel(), v_grid.ravel()
OzL, dzL = lf.ray(uf, vf); OzR, dzR = rf.ray(uf, vf)
OcL, dcL = lf.ray(np.array([1024.]), np.array([1024.]))
OcR, dcR = rf.ray(np.array([1024.]), np.array([1024.]))
WD_est = float(np.mean([float(opt_t[i][2]) for i in range(n_frames)]))
b_est = float(np.linalg.norm(OcR[0] - OcL[0]))
f_obj_est = WD_est - float((abs(OcL[0,2]) + abs(OcR[0,2])) / 2)
theta_fixed = float(np.arctan2(b_est / 2, f_obj_est))

def apply_se3(O, d, rv, t):
    R = Rotation.from_rotvec(rv).as_matrix()
    return (R @ O.T).T + t[None, :], _normalize((R @ d.T).T)

# ═══════════════════════════════════════════════════════════
# Fit 26p full SE(3) model
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Fitting 26p full SE(3) CMO")
print("=" * 60)

def build_full_model(x_tel, arm_params):
    m_tel = CMOTelecentricStereoModel.from_parameter_vector(x_tel, pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    OL, dL = m_tel.ray(uf, vf, "left")
    OR, dR = m_tel.ray(uf, vf, "right")
    rv_L, t_L = arm_params[:3], arm_params[3:6]
    rv_R, t_R = arm_params[6:9], arm_params[9:12]
    OL_a, dL_a = apply_se3(OL, dL, rv_L, t_L)
    OR_a, dR_a = apply_se3(OR, dR, rv_R, t_R)
    return OL_a, dL_a, OR_a, dR_a, m_tel, rv_L, t_L, rv_R, t_R

x0_tel = np.array([f_obj_est, WD_est, b_est, 1024., 1024., f_obj_est, theta_fixed, dcL[0,1],
                   0., 0., 0., 0., 0., 0.], dtype=np.float64)
lo_tel = np.array([1., 1., 0., 0., 0., 20., 0., -0.3, -10., -10., -10., -10., -10., -10.], dtype=np.float64)
hi_tel = np.array([500., 1000., 200., 2048., 2048., 200., 0.5, 0.3, 10., 10., 10., 10., 10., 10.], dtype=np.float64)

def res_full(x):
    x_tel, arm = x[:14], x[14:]
    OL_a, dL_a, OR_a, dR_a, *_ = build_full_model(x_tel, arm)
    blocks = []
    for z in [50., 80.]:
        for O_a, d_a, O_ref, d_ref in [(OL_a, dL_a, OzL, dzL), (OR_a, dR_a, OzR, dzR)]:
            tz_ref = (z-O_ref[:,2])/d_ref[:,2]; P_ref = O_ref + tz_ref[:,None]*d_ref
            tz_mod = (z-O_a[:,2])/d_a[:,2]; P_mod = O_a + tz_mod[:,None]*d_a
            blocks.append((P_ref - P_mod).reshape(-1))
    return np.concatenate(blocks)

rot_lo = np.full(3, -0.08); rot_hi = np.full(3, 0.08)
trans_lo = np.full(3, -3.0); trans_hi = np.full(3, 3.0)
arm_lo = np.concatenate([rot_lo, trans_lo, rot_lo, trans_lo])
arm_hi = np.concatenate([rot_hi, trans_hi, rot_hi, trans_hi])
x0 = np.concatenate([x0_tel, np.zeros(12)])
lo = np.concatenate([lo_tel, arm_lo]); hi = np.concatenate([hi_tel, arm_hi])
sol = least_squares(res_full, x0=x0, bounds=(lo, hi), loss="huber", f_scale=1.0, max_nfev=500,
                     xtol=1e-10, ftol=1e-10, gtol=1e-10)
x_opt = sol.x
OL_a, dL_a, OR_a, dR_a, m_tel_opt, rv_L, t_L, rv_R, t_R = build_full_model(x_opt[:14], x_opt[14:])
rms_26p = float(np.sqrt(np.mean(res_full(x_opt)**2)))
print(f"26p: RMS={rms_26p:.6f} mm  L rot={np.degrees(np.linalg.norm(rv_L)):.2f}°  R rot={np.degrees(np.linalg.norm(rv_R)):.2f}°")
print(f"  t_L={t_L}  t_R={t_R}")

# ═══════════════════════════════════════════════════════════
# PHASE 1 — Modal residual after 26p
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 1 — Modal residual after 26p")
print("=" * 60)

dot_L = np.clip(np.sum(dzL*dL_a, axis=1), -1, 1); dot_R = np.clip(np.sum(dzR*dR_a, axis=1), -1, 1)
ang_L = np.degrees(np.arccos(dot_L)); ang_R = np.degrees(np.arccos(dot_R))
mzL = np.cross(OzL, dzL); maL = np.cross(OL_a, dL_a)
mzR = np.cross(OzR, dzR); maR = np.cross(OR_a, dR_a)
ddL = dzL - dL_a; ddR = dzR - dR_a
dmL = mzL - maL; dmR = mzR - maR
print(f"Dir RMS: L={np.sqrt(np.mean(ang_L**2)):.5f}°  R={np.sqrt(np.mean(ang_R**2)):.5f}°")
print(f"Mom RMS: L={np.sqrt(np.mean(np.linalg.norm(dmL,axis=1)**2)):.5f} mm  R={np.sqrt(np.mean(np.linalg.norm(dmR,axis=1)**2)):.5f} mm")

modes_6 = zernike_modes(6)
xi = 2.0*uf/(W-1)-1.0; zeta = 2.0*vf/(H-1)-1.0
rho = np.sqrt(xi*xi+zeta*zeta)/np.sqrt(2.0); theta = np.arctan2(zeta, xi)
B = np.empty((uf.size, len(modes_6)), dtype=np.float64)
for j, mode in enumerate(modes_6): B[:, j] = eval_real_zernike(mode, rho, theta)
BtB_inv = np.linalg.inv(B.T @ B + 1e-10*np.eye(len(modes_6)))
B_pinv = BtB_inv @ B.T; Bsq = np.sum(B**2, axis=0)

for label, dd, dm in [("L dir", ddL, None), ("R dir", ddR, None), ("L mom", dmL, None), ("R mom", dmR, None)]:
    vec = ddL if "L dir"==label else ddR if "R dir"==label else dmL if "L mom"==label else dmR
    var_tot = float(np.sum(vec**2))
    if var_tot < 1e-16: continue
    contribs = []
    for j, mode in enumerate(modes_6):
        cx, cy, cz = B_pinv @ vec[:,0], B_pinv @ vec[:,1], B_pinv @ vec[:,2]
        frac = float((cx[j]**2+cy[j]**2+cz[j]**2)*Bsq[j])/max(var_tot,1e-16)
        contribs.append((f"Z_{mode.n}^{mode.m}({mode.kind})", mode.n, frac))
    contribs.sort(key=lambda x: -abs(x[2]))
    print(f"\n  {label} (var={var_tot:.5f}):")
    for name, n, frac in contribs[:10]:
        if frac < 0.005: continue
        print(f"    {name:18s} n={n}  {frac*100:5.1f}%")

# Cumulative by order
for label, vec in [("Δd L", ddL), ("Δd R", ddR), ("Δm L", dmL), ("Δm R", dmR)]:
    var_tot = float(np.sum(vec**2))
    print(f"\n  {label} cumulative by order:")
    cum = 0.0
    for order in range(7):
        idxs = [j for j, m in enumerate(modes_6) if m.n == order]
        frac = sum(float((np.sum((B_pinv @ vec[:,c])[idxs]**2))*Bsq[idxs].sum()) for c in range(3))/max(var_tot,1e-16)
        cum += frac
        print(f"    order {order}: {frac*100:5.1f}%  (cum {cum*100:.0f}%)")

# ═══════════════════════════════════════════════════════════
# PHASE 2 — Pixel reprojection + metric check
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 2 — Pixel reprojection")
print("=" * 60)

all_errors = []
for pi in range(n_frames):
    Rm, t = opt_R[pi], opt_t[pi]
    Xw = (Rm @ obj_pts.T).T + t[None, :]; n_plane = Rm[:, 2]
    z_mm = float(t[2])
    dmask_L, dmask_R = detection_counts[pi]
    for k in range(obj_pts.shape[0]):
        for ch, dmask in [("L", dmask_L), ("R", dmask_R)]:
            uv = left_pixels[pi][k] if ch == "L" else right_pixels[pi][k]
            O_tel, d_tel = m_tel_opt.ray(np.array([uv[0]]), np.array([uv[1]]), "left" if ch=="L" else "right")
            rv, t_arm = (rv_L, t_L) if ch == "L" else (rv_R, t_R)
            O_u, d_u = apply_se3(O_tel.reshape(1,3), d_tel.reshape(1,3), rv, t_arm)
            O_u, d_u = O_u[0], d_u[0]
            dn = float(np.dot(d_u, n_plane))
            if abs(dn) > 1e-10:
                tL_val = float(np.dot(t-O_u, n_plane))/dn
                e = float(np.linalg.norm((O_u + tL_val*d_u)-Xw[k]))
                epx = e/max(abs(tL_val),1.0)*FX
                r = np.sqrt((uv[0]-1024)**2 + (uv[1]-1024)**2)
                all_errors.append((pi, ch, int(dmask[k]), r, epx, z_mm, e, abs(tL_val)))

epx_all = np.array([e[4] for e in all_errors])
print(f"Overall: RMS={np.sqrt(np.mean(epx_all**2)):.2f} px  P50={np.percentile(epx_all,50):.2f} px  P95={np.percentile(epx_all,95):.2f} px")

# Metric sanity: e_mm vs e_px
e_mm = np.array([e[6] for e in all_errors])
t_vals = np.array([e[7] for e in all_errors])
epx_from_mm = e_mm / t_vals * FX
print(f"Metric check: e_mm RMS={np.sqrt(np.mean(e_mm**2)):.4f} mm, conversion consistent: {np.allclose(epx_all, epx_from_mm)}")

print(f"\nBy channel:")
for ch in ["L", "R"]:
    epx_ch = np.array([e[4] for e in all_errors if e[1]==ch])
    print(f"  {ch}: RMS={np.sqrt(np.mean(epx_ch**2)):.2f} px  P50={np.percentile(epx_ch,50):.2f} px  P95={np.percentile(epx_ch,95):.2f} px")

print(f"\nBy radius:")
bins = [(0,300),(300,600),(600,900),(900,1200),(1200,1500)]
for r0, r1 in bins:
    epx_r = np.array([e[4] for e in all_errors if r0 <= e[3] < r1])
    if len(epx_r) > 0:
        print(f"  r=[{r0:4d},{r1:4d}): RMS={np.sqrt(np.mean(epx_r**2)):.2f} px  P50={np.percentile(epx_r,50):.2f} px  n={len(epx_r)}")

print(f"\nBy detection:")
for label, mask_val in [("detected", 1), ("completed", 0)]:
    epx_s = np.array([e[4] for e in all_errors if e[2]==mask_val])
    if len(epx_s): print(f"  {label}: RMS={np.sqrt(np.mean(epx_s**2)):.2f} px  P50={np.percentile(epx_s,50):.2f} px  n={len(epx_s)}")

print(f"\nBy Z:")
for pi in range(n_frames):
    epx_f = np.array([e[4] for e in all_errors if e[0]==pi])
    z_f = np.mean([e[5] for e in all_errors if e[0]==pi])
    print(f"  Frame{pi} Z={z_f:.1f}: RMS={np.sqrt(np.mean(epx_f**2)):.2f} px  P50={np.percentile(epx_f,50):.2f} px")

# ═══════════════════════════════════════════════════════════
# PHASE 3 — Minimal extensions
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 3 — Minimal residual blocks on 26p")
print("=" * 60)

block_tests = [
    ("d_Z2", [m for m in modes_6 if m.n == 2], "d"),
    ("d_Z3", [m for m in modes_6 if m.n == 3], "d"),
    ("d_Z1", [m for m in modes_6 if m.n == 1], "d"),
    ("m_Z1", [m for m in modes_6 if m.n == 1], "m"),
    ("m_Z2", [m for m in modes_6 if m.n == 2], "m"),
]

for block_name, block_modes, block_type in block_tests:
    idxs = [list(modes_6).index(m) for m in block_modes if m in modes_6]
    if not idxs: continue
    B_blk = B[:, idxs]; n_coeffs = B_blk.shape[1]

    def make_res_blk(x):
        cL = x[:n_coeffs*3].reshape(n_coeffs, 3)
        cR = x[n_coeffs*3:2*n_coeffs*3].reshape(n_coeffs, 3)
        if block_type == "d":
            ddL_blk = B_blk @ cL; ddL_blk = ddL_blk - np.sum(ddL_blk*dL_a, axis=1, keepdims=True)*dL_a
            dL_new = _normalize(dL_a + ddL_blk)
            ddR_blk = B_blk @ cR; ddR_blk = ddR_blk - np.sum(ddR_blk*dR_a, axis=1, keepdims=True)*dR_a
            dR_new = _normalize(dR_a + ddR_blk)
            OL_new, OR_new = OL_a, OR_a
        else:
            dOL = B_blk @ cL; dOL = dOL - np.sum(dOL*dL_a, axis=1, keepdims=True)*dL_a
            OL_new = OL_a + dOL
            dOR = B_blk @ cR; dOR = dOR - np.sum(dOR*dR_a, axis=1, keepdims=True)*dR_a
            OR_new = OR_a + dOR
            dL_new, dR_new = dL_a, dR_a
        blocks = []
        for z in [50., 80.]:
            for O_n, d_n, O_r, d_r in [(OL_new, dL_new, OzL, dzL), (OR_new, dR_new, OzR, dzR)]:
                tz_r = (z-O_r[:,2])/d_r[:,2]; P_r = O_r + tz_r[:,None]*d_r
                tz_n = (z-O_n[:,2])/d_n[:,2]; P_n = O_n + tz_n[:,None]*d_n
                blocks.append((P_r-P_n).reshape(-1))
        return np.concatenate(blocks)

    x0b = np.zeros(n_coeffs*6); lob = np.full(n_coeffs*6, -0.05); hib = np.full(n_coeffs*6, 0.05)
    sol_b = least_squares(make_res_blk, x0=x0b, bounds=(lob, hib), loss="linear", max_nfev=200)
    rms_b = float(np.sqrt(np.mean(make_res_blk(sol_b.x)**2)))
    impr = (rms_26p - rms_b)/rms_26p*100
    print(f"  +{block_name:6s} ({n_coeffs*6:+d}p): RMS={rms_b:.6f} mm  Δ={impr:+.1f}%")

# ═══════════════════════════════════════════════════════════
# PHASE 4 — Compression Pareto
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 4 — Compression Pareto")
print("=" * 60)

compressions = [
    ("26p full SE(3)", "full_se3"),
    ("23p shared rot", "shared_rot"),
    ("23p shared trans", "shared_trans"),
    ("20p diff only", "diff_only"),
]

results_comp = []
# 26p already done
results_comp.append({"name": "26p full SE(3)", "params": 26, "ray_rms": rms_26p})

# 23p: shared rotation, differential translation
def build_shared_rot(x_tel, arm_params):
    m_tel = CMOTelecentricStereoModel.from_parameter_vector(x_tel, pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    OL, dL = m_tel.ray(uf, vf, "left"); OR, dR = m_tel.ray(uf, vf, "right")
    rv_shared = arm_params[:3]
    t_L = arm_params[3:6]; t_R = arm_params[6:9]
    OL_a, dL_a = apply_se3(OL, dL, rv_shared, t_L)
    OR_a, dR_a = apply_se3(OR, dR, rv_shared, t_R)
    return OL_a, dL_a, OR_a, dR_a

def res_shared_rot(x):
    x_tel, arm = x[:14], x[14:]
    OL_a, dL_a, OR_a, dR_a = build_shared_rot(x_tel, arm)
    blocks = []
    for z in [50., 80.]:
        for O_a, d_a, O_ref, d_ref in [(OL_a, dL_a, OzL, dzL), (OR_a, dR_a, OzR, dzR)]:
            tz_ref = (z-O_ref[:,2])/d_ref[:,2]; P_ref = O_ref + tz_ref[:,None]*d_ref
            tz_mod = (z-O_a[:,2])/d_a[:,2]; P_mod = O_a + tz_mod[:,None]*d_a
            blocks.append((P_ref-P_mod).reshape(-1))
    return np.concatenate(blocks)

arm_lo_sr = np.concatenate([rot_lo, trans_lo, trans_lo])
arm_hi_sr = np.concatenate([rot_hi, trans_hi, trans_hi])
x0_sr = np.concatenate([x0_tel, np.zeros(9)])
lo_sr = np.concatenate([lo_tel, arm_lo_sr]); hi_sr = np.concatenate([hi_tel, arm_hi_sr])
sol_sr = least_squares(res_shared_rot, x0=x0_sr, bounds=(lo_sr, hi_sr), loss="huber", f_scale=1.0, max_nfev=300, xtol=1e-10)
rms_sr = float(np.sqrt(np.mean(res_shared_rot(sol_sr.x)**2)))
results_comp.append({"name": "23p shared rot", "params": 23, "ray_rms": rms_sr})

# 23p: shared translation, differential rotation
def build_shared_trans(x_tel, arm_params):
    m_tel = CMOTelecentricStereoModel.from_parameter_vector(x_tel, pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
    OL, dL = m_tel.ray(uf, vf, "left"); OR, dR = m_tel.ray(uf, vf, "right")
    rv_L = arm_params[:3]; rv_R = arm_params[3:6]
    t_shared = arm_params[6:9]
    OL_a, dL_a = apply_se3(OL, dL, rv_L, t_shared)
    OR_a, dR_a = apply_se3(OR, dR, rv_R, t_shared)
    return OL_a, dL_a, OR_a, dR_a

def res_shared_trans(x):
    x_tel, arm = x[:14], x[14:]
    OL_a, dL_a, OR_a, dR_a = build_shared_trans(x_tel, arm)
    blocks = []
    for z in [50., 80.]:
        for O_a, d_a, O_ref, d_ref in [(OL_a, dL_a, OzL, dzL), (OR_a, dR_a, OzR, dzR)]:
            tz_ref = (z-O_ref[:,2])/d_ref[:,2]; P_ref = O_ref + tz_ref[:,None]*d_ref
            tz_mod = (z-O_a[:,2])/d_a[:,2]; P_mod = O_a + tz_mod[:,None]*d_a
            blocks.append((P_ref-P_mod).reshape(-1))
    return np.concatenate(blocks)

arm_lo_st = np.concatenate([rot_lo, rot_lo, trans_lo])
arm_hi_st = np.concatenate([rot_hi, rot_hi, trans_hi])
x0_st = np.concatenate([x0_tel, np.zeros(9)])
lo_st = np.concatenate([lo_tel, arm_lo_st]); hi_st = np.concatenate([hi_tel, arm_hi_st])
sol_st = least_squares(res_shared_trans, x0=x0_st, bounds=(lo_st, hi_st), loss="huber", f_scale=1.0, max_nfev=300, xtol=1e-10)
rms_st = float(np.sqrt(np.mean(res_shared_trans(sol_st.x)**2)))
results_comp.append({"name": "23p shared trans", "params": 23, "ray_rms": rms_st})

# 20p diff only (from earlier ablation)
results_comp.append({"name": "20p diff only", "params": 20, "ray_rms": 0.0070})  # from autopsy_20p

print(f"\n{'Model':>20s}  {'P':>4s}  {'Ray RMS':>9s}  {'Δ vs 26p':>8s}")
print(f"  {'─'*20}  {'─'*4}  {'─'*9}  {'─'*8}")
for r in results_comp:
    delta = (r["ray_rms"] - rms_26p)/rms_26p*100
    print(f"  {r['name']:>20s}  {r['params']:>4d}  {r['ray_rms']:9.6f}  {delta:+7.1f}%")

# Save
artifact = {
    "description": "26p aligned CMO autopsy + compression",
    "rms_26p_mm": rms_26p,
    "px_rms": float(np.sqrt(np.mean(epx_all**2))),
    "px_p50": float(np.percentile(epx_all, 50)),
    "px_p95": float(np.percentile(epx_all, 95)),
    "dir_rms_deg": {"L": float(np.sqrt(np.mean(ang_L**2))), "R": float(np.sqrt(np.mean(ang_R**2)))},
    "mom_rms_mm": {"L": float(np.sqrt(np.mean(np.linalg.norm(dmL,axis=1)**2))),
                   "R": float(np.sqrt(np.mean(np.linalg.norm(dmR,axis=1)**2)))},
    "compression": results_comp,
    "per_channel": {"L": float(np.sqrt(np.mean(np.array([e[4] for e in all_errors if e[1]=="L"])**2))),
                    "R": float(np.sqrt(np.mean(np.array([e[4] for e in all_errors if e[1]=="R"])**2)))},
    "detected_vs_completed": {
        "detected": float(np.sqrt(np.mean(np.array([e[4] for e in all_errors if e[2]==1])**2))),
        "completed": float(np.sqrt(np.mean(np.array([e[4] for e in all_errors if e[2]==0])**2))),
    },
}
with open(OUT / "autopsy_26p.json", "w") as f:
    json.dump(artifact, f, indent=2)
print(f"\nSaved: {OUT / 'autopsy_26p.json'}")
print("Done!")
