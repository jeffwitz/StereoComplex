#!/usr/bin/env python3
"""Refine the 26p aligned CMO model directly on corner reprojection.

Initializes from the rayfield-fitted 26p parameters + Zernike poses,
then minimizes corner→ray distance for all 3300 observations.
"""
...
# ═══════════════════════════════════════════════════════════
# Fit 26p to rayfield (initialization)
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Step 1 — 26p rayfield fit (initialization)")
print("=" * 60)
...
def res_rayfield(x):
    m_tel, rv_L, t_L, rv_R, t_R = build_full_model(x)
...
sol_rf = least_squares(res_rayfield, x0=x0, bounds=(np.concatenate([lo_tel, arm_lo]), np.concatenate([hi_tel, arm_hi])),
                        loss="huber", f_scale=1.0, max_nfev=500, xtol=1e-10, ftol=1e-10, gtol=1e-10)
x_rf = sol_rf.x
rms_rf = float(np.sqrt(np.mean(res_rayfield(x_rf)**2)))
print(f"Rayfield RMS = {rms_rf:.6f} mm")

# Pixel reprojection BEFORE corner refinement
m_tel_rf, rv_L_rf, t_L_rf, rv_R_rf, t_R_rf = build_full_model(x_rf)
epx_before = []
for pi in range(n_frames):
    ...
    for k in range(obj_pts.shape[0]):
        for uv, ch in [(left_pixels[pi][k], "left"), (right_pixels[pi][k], "right")]:
            O_tel, d_tel = m_tel_rf.ray(np.array([uv[0]]), np.array([uv[1]]), "left" if ch=="left" else "right")
            rv, t_arm = (rv_L_rf, t_L_rf) if ch == "left" else (rv_R_rf, t_R_rf)
            ...
            if abs(dn) > 1e-10:
                ...
                epx_before.append(e/max(abs(tL_val),1.0)*FX)
epx_b = np.array(epx_before)
print(f"Pixel RMS (before corner BA): {np.sqrt(np.mean(epx_b**2)):.2f} px  P50={np.percentile(epx_b,50):.2f} px  P95={np.percentile(epx_b,95):.2f} px")

# ═══════════════════════════════════════════════════════════
# Step 2 — Direct corner bundle adjustment
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Step 2 — Corner bundle adjustment (model + poses)")
print("=" * 60)

# Parameter vector: [tele(14) | arm(12) | pose_rv_L(n_frames×3) | pose_t_L(n_frames×3)]
# Use Zernike poses as initial values, shared between L and R
# Actually: we fit per-frame poses (rotvec + translation) for the LEFT channel,
# and the right channel uses the same board pose (it's the same physical board!)

n_pose_params = n_frames * 6  # rotvec(3) + tvec(3) per frame
x0_corner = np.concatenate([x_rf, np.zeros(n_pose_params)])

# Initialize poses from Zernike estimates, clip to bounds
for pi in range(n_frames):
    rv = Rotation.from_matrix(opt_R[pi]).as_rotvec()
    rv_clipped = np.clip(rv, -0.49, 0.49)  # within ±0.5 rad bounds
    x0_corner[26 + 6*pi : 26 + 6*pi + 3] = rv_clipped
    x0_corner[26 + 6*pi + 3 : 26 + 6*pi + 6] = np.clip(opt_t[pi], -500, 500)

# Bounds for poses — clip initial rotvecs to bounds to avoid rejection
pose_lo = np.concatenate([np.full(3, -0.5), np.full(3, -np.inf)])
pose_hi = np.concatenate([np.full(3, 0.5), np.full(3, np.inf)])
poses_lo = np.tile(pose_lo, n_frames)
poses_hi = np.tile(pose_hi, n_frames)

lo_corner = np.concatenate([np.concatenate([lo_tel, arm_lo]), poses_lo])
hi_corner = np.concatenate([np.concatenate([hi_tel, arm_hi]), poses_hi])

def res_corners(x):
    m_tel, rv_L, t_L, rv_R, t_R = build_full_model(x[:26])
    total = []
    for pi in range(n_frames):
        rv_pose = x[26 + 6*pi : 26 + 6*pi + 3]
        t_pose = x[26 + 6*pi + 3 : 26 + 6*pi + 6]
        R_pose = Rotation.from_rotvec(rv_pose).as_matrix()
        Xw = (R_pose @ obj_pts.T).T + t_pose[None, :]
        n_plane = R_pose[:, 2]
        for k in range(obj_pts.shape[0]):
            for uv, ch in [(left_pixels[pi][k], "left"), (right_pixels[pi][k], "right")]:
                O_tel, d_tel = m_tel.ray(np.array([uv[0]]), np.array([uv[1]]), "left" if ch=="left" else "right")
                ...
                if abs(dn) > 1e-10:
                    ...
                    total.append(float(np.linalg.norm((O_u + tL_val*d_u)-Xw[k])))
                else:
                    total.append(1e3)  # penalty for degenerate rays
    return np.array(total)

print(f"Parameters: 26 model + {n_pose_params} poses = {26 + n_pose_params} total")
print(f"Observations: {n_frames * 165 * 2} = {n_frames * 330}")

t0 = time.time()
# First, fit model only with poses fixed (refine optics)
sol_c1 = least_squares(res_corners, x0=x0_corner,
                        bounds=(lo_corner, hi_corner),
                        loss="soft_l1", f_scale=1.0, max_nfev=1000,
                        xtol=1e-8, ftol=1e-8, gtol=1e-8)
rms_c1 = float(np.sqrt(np.mean(res_corners(sol_c1.x)**2)))
# Convert mm to px
epx_after = res_corners(sol_c1.x) / np.array([np.linalg.norm(opt_t[pi]) for pi in range(n_frames) for _ in range(330)]).mean() * FX
# Actually compute proper pixel-equivalent
m_tel_c, rv_L_c, t_L_c, rv_R_c, t_R_c = build_full_model(sol_c1.x[:26])
epx_after = []
for pi in range(n_frames):
    rv_pose = sol_c1.x[26 + 6*pi : 26 + 6*pi + 3]
    t_pose = sol_c1.x[26 + 6*pi + 3 : 26 + 6*pi + 6]
    R_pose = Rotation.from_rotvec(rv_pose).as_matrix()
    Xw = (R_pose @ obj_pts.T).T + t_pose[None, :]; n_plane = R_pose[:, 2]
    for k in range(obj_pts.shape[0]):
        for uv, ch in [(left_pixels[pi][k], "left"), (right_pixels[pi][k], "right")]:
            O_tel, d_tel = m_tel_c.ray(np.array([uv[0]]), np.array([uv[1]]), "left" if ch=="left" else "right")
            rv, t_arm = (rv_L_c, t_L_c) if ch == "left" else (rv_R_c, t_R_c)
            O_u, d_u = apply_se3(O_tel.reshape(1,3), d_tel.reshape(1,3), rv, t_arm)
            O_u, d_u = O_u[0], d_u[0]
            dn = float(np.dot(d_u, n_plane))
            if abs(dn) > 1e-10:
                tL_val = float(np.dot(t_pose-O_u, n_plane))/dn
                e = float(np.linalg.norm((O_u + tL_val*d_u)-Xw[k]))
                epx_after.append(e/max(abs(tL_val),1.0)*FX)
epx_a = np.array(epx_after)
elapsed = time.time() - t0

print(f"Corner BA: nfev={sol_c1.nfev}, time={elapsed:.0f}s")
print(f"Pixel RMS (after corner BA):  {np.sqrt(np.mean(epx_a**2)):.2f} px  P50={np.percentile(epx_a,50):.2f} px  P95={np.percentile(epx_a,95):.2f} px")
print(f"Pixel RMS (before, rayfield): {np.sqrt(np.mean(epx_b**2)):.2f} px  P50={np.percentile(epx_b,50):.2f} px  P95={np.percentile(epx_b,95):.2f} px")

improvement = (np.sqrt(np.mean(epx_b**2)) - np.sqrt(np.mean(epx_a**2))) / np.sqrt(np.mean(epx_b**2)) * 100
print(f"\nImprovement: {improvement:+.1f}%")

# Compare model parameters
print(f"\nTelecentric params shift:")
for i, name in enumerate(["f_obj", "WD", "b", "cx", "cy", "f_ang", "theta", "d_y", "sx_L", "sy_L", "sx_R", "sy_R", "rho_x", "rho_y"]):
    delta = sol_c1.x[i] - x_rf[i]
    print(f"  {name:8s}: {x_rf[i]:10.4f} → {sol_c1.x[i]:10.4f}  Δ={delta:+.4f}")

# Save
artifact = {
    "description": "26p model refined directly on corner reprojection",
    "before_rayfield": {"px_rms": float(np.sqrt(np.mean(epx_b**2))), "px_p50": float(np.percentile(epx_b,50)), "px_p95": float(np.percentile(epx_b,95))},
    "after_corner_ba": {"px_rms": float(np.sqrt(np.mean(epx_a**2))), "px_p50": float(np.percentile(epx_a,50)), "px_p95": float(np.percentile(epx_a,95))},
