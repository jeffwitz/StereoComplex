#!/usr/bin/env python3
"""Residual autopsy of the 20p differential SE(3) CMO model.

Phases 1-3: modal decomposition, pixel error maps, single-block additions.
"""
...
for z_str in paired_z:
    ...
    # Which corners were detected vs completed
    detected_mask_L = np.zeros(165, dtype=bool)
    detected_mask_R = np.zeros(165, dtype=bool)
    if ids_L is not None:
        detected_mask_L[np.asarray(ids_L).ravel()] = True
    if ids_R is not None:
        detected_mask_R[np.asarray(ids_R).ravel()] = True
    detection_counts.append((detected_mask_L, detected_mask_R))
...
# ═══════════════════════════════════════════════════════════
# Fit 20p differential SE(3) model
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Fitting 20p differential SE(3) CMO")
print("=" * 60)
...
def build_diff_model(x_tel, arm_params):
    """20p model: telecentric + differential SE(3)."""
    ...
    rv_diff = arm_params[:3]; t_diff = arm_params[3:6]
    R_diff = Rotation.from_rotvec(rv_diff).as_matrix()
    # L stays as-is, R gets the differential transform
    OR_a = (R_diff @ OR.T).T + t_diff[None, :]
    dR_a = _normalize((R_diff @ dR.T).T)
    return OL, dL, OR_a, dR_a
...
def res_diff(x):
    x_tel, arm = x[:14], x[14:]
    OL_a, dL_a, OR_a, dR_a = build_diff_model(x_tel, arm)
    blocks = []
    for z in [50., 80.]:
...
x0 = np.concatenate([x0_tel, np.zeros(6)])
lo = np.concatenate([lo_tel, rot_lo, trans_lo])
hi = np.concatenate([hi_tel, rot_hi, trans_hi])
sol = least_squares(res_diff, x0=x0, bounds=(lo, hi), loss="huber", f_scale=1.0, max_nfev=500,
                     xtol=1e-10, ftol=1e-10, gtol=1e-10)
x_opt = sol.x
rv_diff = x_opt[14:17]; t_diff = x_opt[17:20]
rms_20p = float(np.sqrt(np.mean(res_diff(x_opt)**2)))
print(f"20p diff SE(3): RMS={rms_20p:.6f} mm, rot_diff={np.degrees(np.linalg.norm(rv_diff)):.3f}°, t_diff={t_diff}")

# Evaluate rays
m_tel_opt = CMOTelecentricStereoModel.from_parameter_vector(x_opt[:14], pixel_pitch_mm=0.0055, image_size=IMG_SIZE)
OL_a, dL_a, OR_a, dR_a = build_diff_model(x_opt[:14], x_opt[14:])

# ═══════════════════════════════════════════════════════════
# PHASE 1 — Modal residual after 20p
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 1 — Modal residual after 20p diff SE(3)")
print("=" * 60)

# Direction and moment errors
...
print(f"Dir RMS: L={np.sqrt(np.mean(ang_L**2)):.4f}°  R={np.sqrt(np.mean(ang_R**2)):.4f}°")
print(f"Mom RMS: L={np.sqrt(np.mean(np.linalg.norm(dmL,axis=1)**2)):.4f} mm  R={np.sqrt(np.mean(np.linalg.norm(dmR,axis=1)**2)):.4f} mm")
...
for label, dd, dm, var_label in [("L dir", ddL, None, "d"), ("R dir", ddR, None, "d"),
                                   ("L mom", dmL, None, "m"), ("R mom", dmR, None, "m")]:
    var_tot = float(np.sum((ddL if "L dir" in label else ddR if "R dir" in label else dmL if "L mom" in label else dmR)**2))
    ...
    for j, mode in enumerate(modes_6):
        ...
        contribs.append((f"Z_{mode.n}^{mode.m}({mode.kind})", mode.n, frac, float(cx[j]), float(cy[j]), float(cz[j])))
    contribs.sort(key=lambda x: -abs(x[2]))
    print(f"\n  {label} (total var={var_tot:.4f}):")
    for name, n, frac, cx, cy, cz in contribs[:8]:
        if frac < 0.005: continue
        print(f"    {name:18s} n={n}  {frac*100:5.1f}%  c=({cx:+.4f},{cy:+.4f},{cz:+.4f})")

# ═══════════════════════════════════════════════════════════
# PHASE 2 — Pixel residual maps
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 2 — Pixel residual maps")
print("=" * 60)

R_diff_mat = Rotation.from_rotvec(rv_diff).as_matrix()
all_errors = []  # (pi, ch, k, is_detected, uv, epx, z_mm)
...
for pi in range(n_frames):
    ...
    for k in range(obj_pts.shape[0]):
        for ch, uv, dmask in [("L", left_pixels[pi][k], dmask_L), ("R", right_pixels[pi][k], dmask_R)]:
            O_tel, d_tel = m_tel_opt.ray(np.array([uv[0]]), np.array([uv[1]]), "left" if ch=="L" else "right")
            if ch == "R":
                O_use = (R_diff_mat @ O_tel[0]).T + t_diff
                d_use = _normalize(R_diff_mat @ d_tel[0])
            else:
                O_use, d_use = O_tel[0], d_tel[0]
            dn = float(np.dot(d_use, n_plane))
            if abs(dn) > 1e-10:
                tL_val = float(np.dot(t - O_use, n_plane))/dn
                e = float(np.linalg.norm((O_use + tL_val*d_use) - Xw[k]))
                epx = e/max(abs(tL_val),1.0)*FX
                r = np.sqrt((uv[0]-1024)**2 + (uv[1]-1024)**2)
                all_errors.append((pi, ch, int(dmask[k]), r, epx, z_mm))
...
for pi in range(n_frames):
    ...
    z_val = all_errors[0][5] if all_errors else 0
    z_f = np.mean([e[5] for e in all_errors if e[0]==pi])
    print(f"  Frame {pi} (Z={z_f:.1f}mm): RMS={np.sqrt(np.mean(epx_f**2)):.2f} px  P50={np.percentile(epx_f,50):.2f} px  P95={np.percentile(epx_f,95):.2f} px")

print(f"\nBy detection status:")
for label, mask_val in [("detected", 1), ("completed", 0)]:
    epx_s = np.array([e[4] for e in all_errors if e[2]==mask_val])
    if len(epx_s) > 0:
        print(f"  {label}: RMS={np.sqrt(np.mean(epx_s**2)):.2f} px  P50={np.percentile(epx_s,50):.2f} px  n={len(epx_s)}")

print(f"\nBy radius from centre:")
...
# ═══════════════════════════════════════════════════════════
# PHASE 3 — Single-block residual additions
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("PHASE 3 — Single-block residual additions")
print("=" * 60)

# Add one Zernike block at a time to the 20p model
# We add a small direction correction: d = normalize(d_20p + B_block @ c_block)
# For each block, fit the coefficients against the Zernike rayfield

block_tests = [
    ("d_Z2_all", "direction Z2 (defocus+astigmatism)", [m for m in modes_6 if m.n == 2], "d"),
    ("d_Z3_all", "direction Z3 (coma+trefoil)", [m for m in modes_6 if m.n == 3], "d"),
    ("d_Z4_all", "direction Z4 (spherical+...)", [m for m in modes_6 if m.n == 4], "d"),
    ("m_Z2_all", "moment Z2", [m for m in modes_6 if m.n == 2], "m"),
]

# Precompute basis values for each block
def block_basis(modes_block):
    idxs = [list(modes_6).index(m) for m in modes_block if m in modes_6]
    return B[:, idxs], idxs

for block_name, desc, block_modes, block_type in block_tests:
    B_blk, _ = block_basis(block_modes)
    if B_blk.shape[1] == 0: continue
    n_coeffs = B_blk.shape[1]
    # Fit: minimize two-plane residual by adding direction or moment correction
    # For direction: d_corrected = normalize(d_20p + project_transverse(B_blk @ c, d_20p))
    # For moment: O_corrected = O_20p + project_transverse(B_blk @ c, d_20p)

    def res_block(x):
        OL_20 = OL_a.copy(); dL_20 = dL_a.copy()
        OR_20 = OR_a.copy(); dR_20 = dR_a.copy()
        cL = x[:n_coeffs*3].reshape(n_coeffs, 3)
        cR = x[n_coeffs*3:2*n_coeffs*3].reshape(n_coeffs, 3)

        if block_type == "d":
            ddL = B_blk @ cL; ddL = ddL - np.sum(ddL*dL_20, axis=1, keepdims=True)*dL_20
            dL_new = _normalize(dL_20 + ddL)
            ddR = B_blk @ cR; ddR = ddR - np.sum(ddR*dR_20, axis=1, keepdims=True)*dR_20
            dR_new = _normalize(dR_20 + ddR)
            OL_new, OR_new = OL_20, OR_20
        else:  # moment
            dOL = B_blk @ cL; dOL = dOL - np.sum(dOL*dL_20, axis=1, keepdims=True)*dL_20
            OL_new = OL_20 + dOL
            dOR = B_blk @ cR; dOR = dOR - np.sum(dOR*dR_20, axis=1, keepdims=True)*dR_20
            OR_new = OR_20 + dOR
            dL_new, dR_new = dL_20, dR_20
    ...
    x0_block = np.zeros(n_coeffs * 6)
    lo_block = np.full(n_coeffs * 6, -0.05)
    hi_block = np.full(n_coeffs * 6, 0.05)
    sol_blk = least_squares(res_block, x0=x0_block, bounds=(lo_block, hi_block), loss="linear", max_nfev=200)
    rms_blk = float(np.sqrt(np.mean(res_block(sol_blk.x)**2)))
    improvement = (rms_20p - rms_blk) / rms_20p * 100
    print(f"  {desc:40s} ({n_coeffs*6:+d}p): RMS={rms_blk:.6f} mm  Δ={improvement:+.1f}%")

# Save
artifact = {
    "description": "Residual autopsy of 20p differential SE(3) CMO model",
    "rms_20p_mm": rms_20p, "px_rms_20p": float(np.sqrt(np.mean(epx_all**2))),
    "px_p50_20p": float(np.percentile(epx_all, 50)),
    "px_p95_20p": float(np.percentile(epx_all, 95)),
    ...
    "per_frame": [{"frame": pi, "z_mm": float(np.mean([e[5] for e in all_errors if e[0]==pi])),
                   "rms": float(np.sqrt(np.mean(np.array([e[4] for e in all_errors if e[0]==pi])**2))),
                   "p50": float(np.percentile(np.array([e[4] for e in all_errors if e[0]==pi]), 50))}
                  for pi in range(n_frames)],
...
with open(OUT / "autopsy_20p.json", "w") as f:
    json.dump(artifact, f, indent=2)
print(f"\nSaved: {OUT / 'autopsy_20p.json'}")
print("Done!")
