...
def render(manifest: dict, manifest_root: Path, out_dir: Path) -> None:
    ...
    xmax = manifest["residual_histogram_xmax_mm"]
    # Bin both distributions over the (tight) display window so the small
    # after-alignment residual (green) is resolved; the before-alignment
    # residual (red, median in the legend) extends past the window.
    axes[1, 2].hist(dPr, bins=100, range=(0.0, xmax), alpha=0.6, color="red",
                    label=f"before SE(3), raw (med={np.median(dPr):.3f})")
    axes[1, 2].hist(dPo, bins=100, range=(0.0, xmax), alpha=0.6, color="green",
