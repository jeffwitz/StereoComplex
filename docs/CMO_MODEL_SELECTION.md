...
- one angular scale from tube focal length and pixel pitch;
...
| Candidate | Parameters | What it can represent |
|---|---:|---|
| physical CMO stereo | 18 shared | common objective, shared sub-pupil baseline, angular scale, and chief-ray convergence |
...
the left and right measured `O,d` rayfields together. Its 18 parameters are
shared across both channels. The polynomial surrogate is represented by the two
...
| physical CMO shared | 18 | 0.00093 mm | 0.00092 mm | 0.00094 mm | -47778.8 |
...
The individual `f_tube` and pixel pitch values are correlated in ray geometry;
their ratio is the identifiable angular scale. Higher Brown radial terms are
also correlated on this narrow field and should not be over-interpreted. The
stable scientific checks are the shared geometry, stereo baseline, angular
scale, ray-space residuals, and BIC.
...
- uses a paraxial physical CMO model with Brown-Conrady terms, not a full optical
  design with zoom, chromatic effects, or diffraction.
