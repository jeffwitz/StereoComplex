...
\begin{abstract}
Stereo calibration pipelines are highly sensitive to structured photometric degradations such as compression artifacts, blur, or noise, which can bias corner localization and degrade epipolar geometry even when reprojection errors remain low.
We propose a lightweight planar correspondence refinement strategy that regularizes marker-based detections through a robust homography followed by a smooth residual warp, improving geometric consistency prior to stereo calibration.
This refinement is designed as a drop-in preprocessing step for standard pinhole stereo toolchains, leaving downstream rectification, disparity computation, and triangulation code unchanged.
We further introduce an optional compact central ray-based stereo backend, parameterized in a Zernike basis and solved via regularized point–ray optimization, which improves robustness under severe photometric stress compared to pinhole-only calibration.
This ray-based backend optionally replaces pinhole calibration and triangulation when maximum robustness is required.
Extensive benchmarks on a controlled dataset with increasing compression and blur levels show that the proposed approach significantly stabilizes epipolar geometry and downstream stereo performance.
All datasets, code, and evaluation protocols are made publicly available to ensure full reproducibility.
\end{abstract}
...
\begin{itemize}
  \item \textbf{C1 (Planar refinement).} A two-stage planar geometric refinement pipeline (robust homography + smooth residual warp) that improves correspondence consistency under degradations (compression, blur, noise) before stereo calibration.
        This refinement is a drop-in preprocessing step that leaves downstream rectification/disparity code unchanged (Section~\ref{sec:method2d}, Table~\ref{tab:practical_guidelines}).
  ...
  \item \textbf{C4 (Optional ray-based stereo).} A compact \emph{central} ray-based stereo model (Zernike basis + regularized point$\leftrightarrow$ray optimization) that yields improved robustness under compression compared to pinhole-only calibration on our benchmark.
        The ray-based backend optionally replaces pinhole calibration/triangulation, and virtual rectification maps (Section~\ref{sec:method2d}) enable reuse of standard dense matchers when preferred.
\end{itemize}
...
\subsection{Practical stereo toolchains}
Kalibr~\cite{furgale2013unified} and Basalt~\cite{usenko2020vin} provide rich calibration backends (including inertial), but require a robotics stack (ROS/catkin/Docker) and are often heavy to deploy for simple stereo-only setups.
MATLAB toolboxes and OpenCV remain common baselines for industrial stereo calibration, yet typically assume high-quality imagery and focus on minimizing reprojection RMS.
Our method is intended as a lightweight planar refinement that drops into such pipelines without changing downstream rectification/disparity code; an optional ray-based backend can replace the pinhole calibration + triangulation stages when rectification-free ray intersection is preferred.
...
\subsubsection{Connection to rectified stereo pipelines}
In many practical stereo systems, calibration is primarily used to enable stable scanline rectification. Rectification quality is therefore a direct proxy for downstream performance of block matching / semi-global matching and for triangulation from horizontal disparities. For pinhole backends, we explicitly evaluate epipolar consistency through vertical disparity statistics after \texttt{stereoRectify} (median/P95 of $|\Delta y|$). Importantly, this metric is computed by rectifying \textbf{ground-truth correspondences} with the \textbf{estimated} stereo parameters (Table~\ref{tab:compression_summary}), isolating the quality of the estimated geometry from detection noise. This directly answers the practical question: does the estimated calibration produce stable rectified pairs under photometric stress?
...
\paragraph{Integration into a standard stereo pipeline (drop-in step).}
The proposed planar refinement is a preprocessing stage that operates \textbf{only on 2D correspondences on the calibration plane}, before any camera model fitting. It can be inserted as a drop-in replacement for raw ChArUco corner sets in a standard stereo workflow:

- \textbf{Standard}: detect corners $\rightarrow$ stereo calibration (pinhole) $\rightarrow$ \texttt{stereoRectify} $\rightarrow$ triangulation / disparity.
- \textbf{With planar refinement}: detect ArUco markers (and/or corners) $\rightarrow$ \textbf{planar refinement (H + TPS residual)} $\rightarrow$ stereo calibration (same backend) $\rightarrow$ \texttt{stereoRectify} $\rightarrow$ triangulation / disparity.

Downstream steps (rectification, disparity computation, triangulation) remain unchanged; only the input correspondence set is regularized.
For the ray-based backend, correspondences can come either from sparse features or from dense \emph{2D disparities} (e.g., produced by a dense matcher); triangulation is performed by ray intersection without requiring pinhole scanline rectification. Virtual rectification maps (not used in our main experiments) are available to reuse off-the-shelf 1D dense matchers on scanline-aligned images when desired.

\paragraph{Implementation note (virtual rectification maps).}
Virtual rectification is implemented as a dense warp toward a rectified virtual pinhole camera, producing \texttt{mapx/mapy} lookup tables for \texttt{cv::remap}. Since our ray-field is defined in the forward direction (pixel $\rightarrow$ ray direction; Eq.~(5)), building the warp requires inverting the mapping (direction $\rightarrow$ pixel). In practice we use a coarse inverse lookup table (LUT) to initialize the solution, followed by a few Gauss--Newton iterations that minimize the angular discrepancy between the target direction and the ray-field direction. This enables reuse of standard scanline dense matchers (e.g., SGM/Census) on the rectified images when desired.
...
This ray-based backend is \textbf{optional}: it can replace the classical pinhole calibration + triangulation stages when maximum robustness is required, while the planar refinement remains compatible with standard rectified stereo pipelines.
We parameterize
\begin{equation}
...
\begin{itemize}
  ...
  \item \textbf{B3: 3D ray-field + planar refinement.} Apply planar refinement to regularize 2D correspondences, then \textbf{replace} pinhole stereo calibration/triangulation with the compact central ray-based stereo model and ray-intersection triangulation (no scanline rectification required).
...
\subsection{Quantitative uncertainty comparison (pinhole vs. ray-field)}
To compare the two backends on equal footing we express their uncertainty in an angular form and then propagate it to depth. The pinhole backend uses the reported triangulation RMS ($u_Z$) to recover an equivalent angular uncertainty
\[
u_{\alpha,\mathrm{pinhole}} \;\approx\; \frac{B}{Z^2}\frac{u_Z}{\sqrt{2}},
\]
while the ray-field backend uses the measured ray skew $s$ to obtain
\[
u_{\alpha,\mathrm{ray}} \;\approx\; \frac{s}{Z}.
\]
The depth uncertainty shares the same propagation,
\[
u_Z \;\equiv\; \frac{Z^2}{B}\sqrt{2}\,u_\alpha,\qquad
U_Z \equiv 2u_Z,
\]
so the two backends are compared in the same units. In Table~\ref{tab:uncertainty_comparison} we list the resulting angular uncertainties (mrad) and relative depth uncertainty $U_Z/Z$ for PNG lossless and WebP$q=70$. The ray-field values use the P95 skew to keep the comparison conservative, whereas the pinhole pipeline relies on RMS depth error; both quantities are propagated through the same formulas. The ray-field remains under one tenth of the pinhole uncertainty under both conditions, yielding a depth uncertainty gain above $15\times$ in the compression-heavy regime. This comparison confirms that the ray-based backend keeps geometric uncertainty low even when 2D pixel noise grows. Codec quality is not strictly monotonic—artifact structure can vary with codec parameters—so we keep the two conditions separate rather than interpolating intermediate quality levels.
\input{tables/uncertainty_comparison.tex}

\subsection{Practical deployment guidelines}
Table~\ref{tab:practical_guidelines} summarizes recommended backends by scenario. When images are clean and time-to-calibrate matters most, a standard pinhole pipeline suffices. Under mild photometric stress (e.g., JPEG/WebP $\gtrsim 90$), inserting the planar refinement (H + TPS residual) before calibration significantly stabilizes epipolar geometry at negligible runtime cost. Under strong photometric stress or when rectification-free triangulation is preferred, the ray-based backend offers maximum robustness; a virtual rectification is available to reuse 1D dense matchers if needed.
\begin{table}[H]
\centering
\small
\setlength{\tabcolsep}{4pt}
...
\midrule
Clean imagery, fast calib. & Pinhole (standard) & Minimal cost, sufficient accuracy \\
Mild compression / blur & Pinhole + planar refinement & Stabilizes epipolar geometry cheaply \\
Strong compression / complex optics & Ray-based (optional virtual rectif.) & Robustness; scanline rectif. optional \\
\bottomrule
\end{tabular}
\caption{Practical deployment recommendations. Planar refinement is a drop-in preprocessing of correspondences; the ray-based backend replaces pinhole calibration/triangulation when maximum robustness is required.}
\label{tab:practical_guidelines}
\end{table}

\section{Reproducibility and open-source release}
All experiments in this paper are reproducible from the released repository (\href{https://github.com/jeffwitz/StereoComplex}{StereoComplex}) and documented online (\href{https://stereocomplex.readthedocs.io/en/latest/}{ReadTheDocs}).
Key scripts include:
\begin{itemize}
  ...
  \item Quantitative uncertainty analysis script (\protect\path{experiments/quantitative_uncertainty.py}) that reproduces Table~\ref{tab:uncertainty_comparison}.
\end{itemize}
...
From a practitioner perspective, the planar refinement is a drop-in upgrade to standard pinhole pipelines: it replaces the raw correspondence set and leaves rectification/disparity/triangulation code unchanged. The ray-based backend is optional and can replace pinhole calibration + triangulation when maximum robustness is required; virtual rectification maps are available to reuse standard 1D dense matchers when desired.
\appendix
