...
OpenCV provides widely used implementations~\cite{bradski2000opencv,opencv}, typically optimizing parameters by minimizing reprojection error.
...
For ground-truth evaluation, we therefore report ray-based triangulation error after a best-fit similarity alignment (Sim(3), i.e., rotation, translation, and isotropic scale) between reconstructed points and ground truth.
...
\toprule
Codec & \shortstack{Ray-field reproj.\\RMS L (px)} & \shortstack{Ray-field reproj.\\RMS R (px)} & \shortstack{Ray skew\\P95 (mm)}\\
\midrule
...
\bottomrule
\end{tabular}
\caption{Ray-based diagnostics for the compact 3D ray-field on selected codecs (2D planar refinement enabled). ``Reproj. RMS'' reports per-view reprojection error after origin-preserving similarity alignment (see Section~\ref{sec:ray3d}); this alignment is used only to compare to ground-truth 3D points and does not affect ray skew or baseline metrics computed in the camera frame. ``Ray skew'' is the minimum distance between the two rays for a correspondence, reported as P95 over all points/frames.}
...
A compact central ray-based stereo backend further improves robustness to compression on our benchmark.
