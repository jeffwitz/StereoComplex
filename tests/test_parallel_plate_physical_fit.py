...
def test_parallel_plate_fit_recovers_oracle_parameters_without_noise():
    ...
    result = sc.fit_parallel_plate_to_zernike_rayfield(
...
def test_parallel_plate_ray_lines_are_invariant_to_d1():
    K = _camera_matrix()
    params_a = sc.PinholeParallelPlateFitParams(alpha_deg=12.0, beta_deg=5.0, thickness_mm=8.0, eta=1.5, d1_mm=20.0)
    params_b = sc.PinholeParallelPlateFitParams(alpha_deg=12.0, beta_deg=5.0, thickness_mm=8.0, eta=1.5, d1_mm=140.0)
    ...
    residuals = sc.rayfield_two_plane_residuals(field_a, field_b, pixels, z_planes=(100.0, 1000.0))
    assert np.sqrt(np.mean(residuals**2)) < 1e-9


def test_fitted_plate_from_zernike_rayfield_improves_reconstruction():
    dataset = sc.make_default_parallel_plate_dataset(noise_std_px=0.0)
    report = sc.run_parallel_plate_origin_field_benchmark(max_order=4, noise_std_px=0.0)
    support_left = np.concatenate(dataset.left_pixels, axis=0)
    support_right = np.concatenate(dataset.right_pixels, axis=0)

    plate_left = sc.fit_parallel_plate_to_zernike_rayfield(
    ...
    plate_right = sc.fit_parallel_plate_to_zernike_rayfield(
    ...
    plate_comparison = sc.compare_3d_reconstruction_with_without_origin_field(dataset, None, plate_model)
