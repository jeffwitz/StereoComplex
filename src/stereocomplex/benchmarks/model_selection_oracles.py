...
def build_pinhole_n_camera_oracle(
    ...
    """Build a synthetic N-camera pinhole oracle with known intrinsics.
    ...
    channel_names : list of str
        Names of the channels to simulate (e.g., ["left", "right"]).
...
def build_exotic_zernike_oracle(image_size=_IMAGE_SIZE, seed=_SEED) -> StereoOracle:
    """Build an oracle with low-amplitude high-order Zernike aberrations.

    Deliberately outside standard physical families — tests that BIC correctly
    rejects all compact models for this oracle.
    ...
    image_size : (int, int)
        Sensor dimensions in pixels.
    seed : int
        RNG seed for reproducible aberration patterns.
...
def build_all_oracles(image_size=_IMAGE_SIZE, seed=_SEED) -> list[StereoOracle]:
    ...
    image_size : (int, int)
        Sensor dimensions in pixels.
    seed : int
        RNG seed for reproducible oracle generation.
