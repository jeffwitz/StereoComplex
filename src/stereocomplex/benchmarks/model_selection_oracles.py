...
def build_pinhole_n_camera_oracle(
    ...
    """Build a synthetic N-camera pinhole oracle with known per-channel intrinsics.

    Generates Zernike rayfield data as if observed through N pinhole cameras
    with given camera matrices, stereo baselines, and board poses.  Produces
    a measured rayfield that a pinhole candidate can recover exactly (modulo
    numerical noise), serving as the ground-truth baseline for BIC model selection.

    Parameters
    ----------
    Ks : dict[str, ndarray]
        Camera matrix per channel, each shape (3, 3).
    baselines : dict[(str,str), ndarray]
        Baseline translation per channel pair, shape (3,), in mm.
    board_poses : list of ndarray
        Board poses, each shape (4, 4) homogeneous transform.
    image_size : (int, int)
        Sensor dimensions in pixels.
    resolution : (int, int)
        Rayfield evaluation resolution.

    Returns
    -------
    ZernikeRayField
        Synthetic measured rayfield.
    """
...
def build_exotic_zernike_oracle(image_size=_IMAGE_SIZE, seed=_SEED) -> StereoOracle:
    """Build a synthetic oracle with low-amplitude high-order Zernike aberrations.

    This oracle is deliberately outside the standard physical model families
    (pinhole, Brown-Conrady, parallel-plate, CMO).  It tests whether the BIC
    correctly identifies that no compact model fits — the correct answer is
    to select the full Zernike rayfield rather than any physical candidate.
    ...
    -------
    ZernikeRayField
        Synthetic measured rayfield with exotic Zernike content.
    """
    w, h = image_size
    cx, cy = (w - 1) / 2, (h - 1) / 2
...
def build_all_oracles(image_size=_IMAGE_SIZE, seed=_SEED) -> list[StereoOracle]:
    """Return the six standard oracles in a fixed, documented order.

    The oracles cover: pinhole, Brown-Conrady, parallel-plate, CMO 14p,
    CMO 26p, and exotic Zernike.  Each oracle is a measured Zernike rayfield
    generated by the corresponding physical model.
    ...
    Returns
    -------
    list of ZernikeRayField
        The six standard oracles, in the order above.
    """
    return [
