...
**Minor-revision pass — done.** Reviewer #3: SE(3) arm-alignment schematic added as Figure 7 in §3.5.6 (`figure_se3_arm_alignment/`, `generate_fig_se3_arm_alignment.py`). Reviewer #1: computational-cost mention added in §6.7 (Soloff fit ~few ms vs rayfield-to-CMO identification ~20 s on one CPU core, from the versioned pose_sweep/se3_ablation timings). Reviewer #2 (corner-noise sensitivity study) was deliberately NOT requested by JFW and is not done. **Stage-prior interpretation clarified (2026-06-11, commit dfaa201):** near-fixed ladder reframed as nominal-ladder/Soloff-equivalent limit; absolute stage accuracy (~10%) distinguished from repeatability/jitter; profilometry explicitly stated as external to hyperparameter choice; weak-hierarchy worsening explained; Zernike sweep scope and O(2)+d(2) field-resolved usage clarified; parameter-counting convention added; abstract lightened.
